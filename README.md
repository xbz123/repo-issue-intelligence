...
uv run rii benchmark benchmarks/cases.json \
  ...
  --output benchmarks/results/deterministic-v0.27-batch6-110-cases-run1.json
...
character caps. Current manifest version 14 embeds 110 complete Issue
snapshots across 49 repositories, corrected pre-fix SHAs, and 111 manually reviewed symbol targets
across 81 cases. Older deterministic and DeepSeek artifacts remain committed as historical
...
The current frozen benchmark contains 110 closed issues with linked fix PRs across 49 projects:
17 main, 11 calibration, and 82 generalization cases. It records 164 reviewed production-file
targets and 111 reviewed symbols across 81 cases. Each case uses a committed Issue snapshot and the
parent of the first ordered fix-PR commit as its pre-fix SHA. Only Git-tracked files are eligible
for candidate retrieval.

Three manifest-v14 deterministic v0.27 runs completed 110/110 cases and produced identical
candidates, symbols, and metrics after excluding timestamps, elapsed fields, and cache provenance.
File Recall@1 is `0.3473`, Recall@5 `0.6168`, Recall@10 `0.7168`, Recall@20 `0.8550`, and MRR
`0.5516`. Symbol Recall@1 is `0.2284`, Recall@5 `0.3693`, Recall@10 `0.4002`, Recall@20 `0.4619`,
and symbol MRR `0.3366`. The sixth reviewed batch adds four audited multi-file fixes and three new
repositories, while raising the number of production targets absent from Top-20 to 30.
...
The first six 200-case expansion batches accept 60 manually reviewed cases; multi-file coverage is
37/110. The sixth batch rejects a Yarl documentation-only follow-up and a discussion-driven h11
feature while retaining four stronger multi-file defects. Thirty of the 164 reviewed production
targets remain outside the deterministic Top-20, and the current index provides
file-only symbol ground truth for TypeScript, Rust, and C. Superseded manifests and retained
50-case DeepSeek runs remain provenance only and must not be mixed with manifest-v14 metrics.
