...
G1 release acceptance is complete. The subsequent
[HTTP retry/recovery integration](protocol-v2-http-retry.md) has separate validation.
...
permission-revocation tests were pending at PR73. PR73 merged this review slice only;
the subsequent [HTTP retry delivery](protocol-v2-http-retry.md) covers that integration.
G1 acceptance remains a separate gate.
