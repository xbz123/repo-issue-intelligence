...
Three complete deterministic v0.19 evaluations produced identical candidates, symbols, and metrics
after excluding timestamps and elapsed fields. File Recall@1 is `0.4067`, Recall@5 `0.6900`,
Recall@10 `0.7800`, Recall@20 `0.9900`, and MRR `0.6038`. The 33 labeled cases
...
After the v0.19 bounded reverse-import retrieval change, 61 of 62 reviewed production-file targets
appear in the deterministic Top-20. The remaining miss is Poetry's
`src/poetry/utils/env/python/manager.py` in `poetry-empty-conda-prefix`; future retrieval work should
target that import/runtime-selection boundary without weakening the frozen-case acceptance rules.
