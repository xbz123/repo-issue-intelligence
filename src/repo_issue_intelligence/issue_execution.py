...
def process_issue(
    ...
    *,
    ...
    result = store.get_issue(run.run_id, issue.number)
    if result.deterministic_report is None:
        store.set_deterministic_state(run.run_id, issue.number, "running", resume=resume)
        try:
        ...
        report = InvestigationReport.model_validate(dict(result.deterministic_report))
    ...
    evidence = store.read_evidence(run.run_id, issue.number)
    if evidence is None:
    ...
    return analyze_sealed_issue(
        run, issue, store, llm_analyzer, before_provider_case=before_provider_case
    )


def analyze_sealed_issue(
    run: RunV2,
    issue: IssueRecord,
    store: AgentStoreV2,
    ...
    *,
    ...
    """Analyze only persisted Issue/report/evidence, without reopening source files."""
    result = store.get_issue(run.run_id, issue.number)
    report = InvestigationReport.model_validate(dict(result.deterministic_report))
    evidence = store.read_evidence(run.run_id, issue.number)
    if evidence is None:
        raise StoreError("Analysis requires sealed evidence")
    ...
    with ExitStack() as lifetimes:
        for ordinal in range(policy["max_attempts"]):
            ...
            if getattr(llm_analyzer, "local_execution_scope_v2", None) == "synchronous-http-v1":
                lifetimes.enter_context(store.execution_lock(attempt_id, create=True))
            ...
            try:
                ...
                terminal = AttemptTerminalFields(
                    ...
                    error=AttemptError(
                        ...
                        detail="Remote outcome is unknown"
                        if uncertain
                        else "Provider analysis failed",
                    ),
...
def capture_execution_configuration(
    llm_analyzer: IssueAnalyzerV2 | None,
    *,
    top_k: int,
    max_evidence_chars: int | None,
    max_evidence_lines: int | None,
    max_attempts: int,
    parameter_origins: Mapping[str, str] | None = None,
):
    """Capture the same current execution configuration for new runs and recovery checks."""
    ...
    configuration = capture_requested_run_configuration(
        ...
        parameter_origins={
            name: origin
            for name, origin in (parameter_origins or {}).items()
            if name not in {"budget.output_tokens", "budget.timeout_seconds"}
        },
        protocol={
            "top_k": top_k,
...
def run_agent_v2(
    ...
    *,
    ...
    configuration = capture_execution_configuration(
        llm_analyzer,
        top_k=top_k,
    ...
    with store.writer_lock():
        if parent_run_id is not None and store.get_run(parent_run_id) is None:
            raise StoreError("Parent run is unavailable")
        snapshot = capture_repository_context(repository_root, mode=capture_mode)
        run = store.create_run(
            snapshot, configuration, inputs, run_id=run_id, parent_run_id=parent_run_id
        )
        try:
