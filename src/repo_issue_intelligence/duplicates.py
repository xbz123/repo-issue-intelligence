...
STOPWORDS = {"the", "a", "an", "and", "or", "to", "of", "in", "on", "is", "it", "for", "with", "when"}


def _tokens(issue: IssueRecord) -> set[str]:
    return {word for word in re.findall(r"[a-zA-Z0-9_./-]{3,}", issue.text) if word not in STOPWORDS}
...
def detect_duplicates(issues: list[IssueRecord], threshold: float = 0.55) -> list[DuplicateMatch]:
    ...
    for index, issue in enumerate(issues):
        for candidate in issues[index + 1 :]:
            ...
            if score >= threshold:
                matches.append(DuplicateMatch(issue_number=issue.number, candidate_issue_number=candidate.number, similarity=score, shared_terms=shared))
