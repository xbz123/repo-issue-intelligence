...
def test_real_benchmark_manifest_has_expected_project_tiers() -> None:
    ...
    assert sum(case.tier is BenchmarkTier.GENERALIZATION for case in manifest.cases) == 112
    assert len({case.repository for case in manifest.cases}) == 56
    ...
    assert sum(len(case.expected_files) for case in manifest.cases) == 205
    assert sum(len(case.expected_files) > 1 for case in manifest.cases) == 45
    assert sum(bool(case.expected_symbols) for case in manifest.cases) == 101
    assert sum(len(case.expected_symbols) for case in manifest.cases) == 135
